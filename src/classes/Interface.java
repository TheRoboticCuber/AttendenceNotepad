...
//	public static boolean[][] permissions;
	...
	public static Dictionary<Integer, String[]> initPeopleList() {
		...
		try {
			...
			bufferedReader = new BufferedReader(new FileReader(inputFileName));
			while((line = bufferedReader.readLine())!= null) {
				if (!getValueFromString(line, 3).contentEquals("ID")) {
					String[] person = {getValueFromString(line, 0) , getValueFromString(line, 1), "F", "", "", ""};
					people.put(getIntFromString(getValueFromString(line, 3)), person);
				}
			}
			//return list
			return people;
	...
	public static boolean signInOut(int ID) {
    	...
    	for (int IDNum : IDList) {
    		if (IDNum==ID) {
    			String[] person = (String[]) people.get(ID);
    			if (person[2].contentEquals("T") && person[4].contentEquals("")) {
    				person[2] = "F";
    				person[4] = time;
    				outputToLog(ID, false);
    				return true;
    			}
    			else if (person[2].contentEquals("F") && person[3].contentEquals("")) {
    				person[2] = "T";
    				person[3] = time;
    				person[5] = "1";
    				outputToLog(ID, true);
	...
	public static void outputFinal() {
		...
		try {
			...
			bufferedWriter.write("ID,Full Name,Time Signed In,Time Signed Out,Has Attended Meeting,");
			bufferedWriter.newLine();
			...
			for (Map.Entry<String, Integer> entry : sorted.entrySet()) {
				ID = entry.getValue();
				person = (String[]) people.get(ID);
				bufferedWriter.write(ID + "," + person[0] + " " + person[1] + "," + person[3] + "," + person[4] + "," + person[5] + ",");
				bufferedWriter.newLine();
			}
		} catch (IOException e) {
			e.printStackTrace();
